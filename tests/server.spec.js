...
describe("Operaciones CRUD de cafes", () => {
  ...
  it("Tests ruta GET /cafes, código 200 + ser objeto + contener 1 obj al menos", async () => {
    const { statusCode, body: cafes } = await request(server).get("/cafes"); //Extraemos el statuscode y el arreglo con los datos de cafes de la respuesta de la API y lo renombramos
  ...
  it("Test ruta POST /cafes, café agregado + código 201", async () => {
    ...
    expect(statusCode).toBe(201); //Comprobamos que el statuscode devuelto es 200
  });
  it("Test ruta PUT /cafes:id, error 400 enviando ID en parámetros diferente a ID en payload", async () => {
    ...
    expect(statusCode).toBe(400); //Comprobamos que el statuscode devuelto es 400
  });
});
